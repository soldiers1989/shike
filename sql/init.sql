...
CREATE TABLE `sk_user` (
  ...
  `type` tinyint(3) NOT NULL COMMENT '类型，1是试客，2是商家，3是管理员',
   audit_status tinyint(1) NOT NULL default 0 COMMENT '审核状态 0:未审核  1:通过  2:不通过 (试客用户使用)',
   audit_time datetime NOT NULL COMMENT '审核时间',
   is_test tinyint(3) NOT NULL default 0 COMMENT '是否测试账号，1是测试账号，0是一般账号',
  `created_time` datetime NOT NULL COMMENT '创建时间',
  `updated_time` datetime NOT NULL COMMENT '更新时间',
...
CREATE TABLE `sk_dianpu` (
  ...
  `created_time` datetime NOT NULL COMMENT '创建时间',
  `updated_time` datetime NOT NULL COMMENT '更新时间',
  `is_del` int(11) DEFAULT NULL COMMENT '状态，0是正常，1是删除',
  PRIMARY KEY (`id`)
);
...
CREATE TABLE `sk_baobei` (
  ...
  `sale_price` decimal(10,2) DEFAULT NULL COMMENT '宝贝返款',
  `real_price` decimal(10,2) DEFAULT NULL COMMENT '真实价格',
  `fankuanlv`  decimal(10,2) DEFAULT NULL COMMENT '返款率',
  ...
  `is_limit` int(11) DEFAULT NULL COMMENT '是否限制，1是限制，0是不限制',
  `hdtypeid` int(11) DEFAULT NULL COMMENT '活动类型id,1：爆款打造、2：新品提升、3：高单、4:必中、5：拼团、7：折扣试用',
