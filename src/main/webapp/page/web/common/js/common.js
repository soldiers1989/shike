    ...
    if (username.val() == '') {
        ...
        $.post('/CommonBase/LoginSubmit', { username: username.val(), password: password.val(), type: type, returnurl: returnurl }, function (d) {
            if (d.success) {
                ...
                location.href = "/Login/BindAccount?userId=" + d.userid;
            } else {
...
function alertRaMsg(i) {
    ...
    myDialog({
        ...
        okText: "开通会员",
        ok: function () { location.href = '/VIP/ApplyVip'; }
    });
}
...
function addbalcklist(userid) {
    if (confirm("如虚假单号、恶意退款用户可加入黑名单。加入黑名单后，该用户将不能申请您发布的活动")) {
        $.post('/BlackList/Add', { userId: userid }, function (d) {
            myAlert(d.Message);
        });
    }
}
...
function initCheckAll() {
    if ($("#chkall").length > 0) {
        $("#chkall").click(function () {
            if (this.checked) {
                $("input[name='id']").each(function () { if (!this.disabled) { this.checked = true; }; });
            } else {
                $("input[name='id']").each(function () { if (!this.disabled) { this.checked = false; }; });
            }
        });
    }
...
function myDialog(options) {
    ...
    var defaults = {
        ...
        okText: "确定",
        cancelText: null,
        ok: function () { $('#linglamsg').remove(); },
        cancel: function () { $('#linglamsg').remove(); },
        close: function () { $('#linglamsg').remove(); }
    };
    ...
    var html = '<div class="tchs" id="linglamsg">' +
                    ...
                    '<div id="msgbox" class="lila_index" style="position:absolute;top:50%;left:50%;">' +
                        '<div class="ling">' +
                            '<strong><img src="/images/lila.jpg" /></strong> ' +
                            '<a style="cursor: pointer"><img src="/images/xx.jpg" /></a>' +
                        '</div>' +
                        '<div class="ling_mian">' +
...
function myConfirm(message, callback, callback2, okText, cancelText) {
    $('#linglamsg').remove();
    var html = '<div class="tchs" id="linglamsg">' +
                    '<div style="position:relative;width:100%;height:100%">' +
                    '<div id="msgbox" class="lila_index" style="position:absolute;top:50%;left:50%;">' +
                        '<div class="ling">' +
                            '<strong><img src="/images/lila.jpg" /></strong> ' +
                            '<a onclick="$(\'#linglamsg\').remove()" style="cursor: pointer"><img src="/images/xx.jpg" /></a>' +
                        '</div>' +
                        '<div class="ling_mian">' +
                            '<div class="sy_cgt">' + message + '</div>' +
                            '<a href="javascript:" class="qdingann qdingann_a" >' + (okText || '确定') + '</a>' +
                            '<a href="javascript:" class="qdingann qdingann_b" style="background-color:#999;">' + (cancelText || '取消') + '</a>' +
                        '</div>' +
                    '</div>' +
                '</div>' +
                '</div>';
...
function myPrompt(callback) {
    ...
    var html = '<div class="tchs" id="linglamsg">' +
                    ...
                    '<div id="msgbox" class="lila_index" style="position:absolute;top:50%;left:50%;">' +
                        '<div class="ling">' +
                            '<strong><img src="/images/lila.jpg" /></strong> ' +
                            '<a style="cursor: pointer" onclick="$(\'#linglamsg\').remove()"><img src="/images/xx.jpg" /></a>' +
                        '</div>' +
                        '<div class="ling_mian">' +
                            '<div class="sy_cgt">' +
